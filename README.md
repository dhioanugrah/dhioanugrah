<!-- Banner -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=0A0A0A&height=120&text=Hi%2C%20I'm%20Dhio%20Anugrah%20👋&fontSize=36&fontColor=FFFFFF&desc=Backend%20Developer%20%7C%20Laravel%20%2B%20Vue%20%7C%20AI%20Integration%20Enthusiast&descAlignY=70" alt="banner"/>

### 🧑‍💻 About Me
Backend Developer & student at **Institut Teknologi Kalimantan (ITK)**.  
Berfokus pada **Laravel**, **Vue**, dan **data-driven system**, dengan passion di **Learning Management System** & **warehouse management**.  
Pernah memimpin proyek sistem manajemen warehouse di PT Hidrolik Teknologi Indotama, serta aktif mengembangkan model AI.

---

### ⚙️ Tech Stack
![Laravel](https://img.shields.io/badge/-Laravel-0A0A0A?style=for-the-badge&logo=laravel&logoColor=FF2D20)
![PHP](https://img.shields.io/badge/-PHP-0A0A0A?style=for-the-badge&logo=php&logoColor=777BB4)
![Vue](https://img.shields.io/badge/-Vue-0A0A0A?style=for-the-badge&logo=vuedotjs&logoColor=41B883)
![Vite](https://img.shields.io/badge/-Vite-0A0A0A?style=for-the-badge&logo=vite&logoColor=646CFF)
![TailwindCSS](https://img.shields.io/badge/-TailwindCSS-0A0A0A?style=for-the-badge&logo=tailwindcss&logoColor=38B2AC)
![MySQL](https://img.shields.io/badge/-MySQL-0A0A0A?style=for-the-badge&logo=mysql&logoColor=4479A1)
![Python](https://img.shields.io/badge/-Python-0A0A0A?style=for-the-badge&logo=python&logoColor=FFD43B)
![Looker](https://img.shields.io/badge/-Looker-0A0A0A?style=for-the-badge&logo=looker&logoColor=4285F4)
![Docker](https://img.shields.io/badge/-Docker-0A0A0A?style=for-the-badge&logo=docker&logoColor=2496ED)

---

### 🚀 Highlighted Projects
#### 🏭 Warehouse Management System – Laravel + Filament
Project Manager & Backend Dev. Sistem untuk manajemen stok & barang gudang dengan role-based access, dashboard, dan approval flow.

#### 🧠 E-Commerce with AI Sentiment Analysis – Laravel + Vue
Sistem penjualan online yang terintegrasi AI untuk analisis sentimen review produk.  
Memanfaatkan model NLP Python untuk menilai positif/negatif feedback pelanggan & menampilkan hasilnya langsung di dashboard.

#### 💧 Mantan Shoesclean API – REST API System
RESTful API untuk pengelolaan pelanggan dan pesanan layanan cuci sepatu, menggunakan trigger dan function SQL untuk integritas data.

#### 🎓 Online Learning Platform – Full-stack
Backend berbasis Laravel + JSX API komunikasi frontend, dengan uji endpoint via Postman dan pengelolaan materi modular.

#### 🏫 Admission System – Summit ITK
Sistem penerimaan mahasiswa baru jalur mandiri berbasis Laravel (kerjasama dengan dosen ITK).

---

### 📊 GitHub Stats
<p align="center">
  <img height="160" src="https://github-readme-stats.vercel.app/api?username=dhioanugrah&show_icons=true&hide_border=true&theme=github_dark" />
  <img height="160" src="https://streak-stats.demolab.com?user=dhioanugrah&theme=github-dark&hide_border=true" />
</p>
<p align="center">
  <img height="120" src="https://github-readme-stats.vercel.app/api/top-langs/?username=dhioanugrah&layout=compact&theme=github_dark&hide_border=true" />
</p>

---

### 🧩 Activities & Leadership
- 🪶 *Ketua Umum MAPALA DEWA ITK* – Memimpin organisasi pengembangan mahasiswa  
- 💼 *Staff Ahli PSDM Himpunan* – Pengembangan kapasitas dan potensi mahasiswa  

---

### 📫 Reach Me
<p align="center">
  <a href="mailto:dhioanugrahh@gmail.com">
    <img src="https://img.shields.io/badge/Email-0A0A0A?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="https://linkedin.com/in/dhioanugrah">
    <img src="https://img.shields.io/badge/LinkedIn-0A0A0A?style=for-the-badge&logo=linkedin&logoColor=0A66C2" />
  </a>
  <a href="https://github.com/dhioanugrah">
    <img src="https://img.shields.io/badge/GitHub-0A0A0A?style=for-the-badge&logo=github&logoColor=white" />
  </a>
</p>

---

